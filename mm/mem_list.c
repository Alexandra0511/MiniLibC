...
int mem_list_add(void *start, size_t len)
{
	...
	// if (mem_list_head.start == NULL) {
	// 	mem_list_head.start = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	// 	return 0;
	// }
	item = mem_list_alloc();
	if (item == NULL)
	...
	item->start = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	item->len = len;
